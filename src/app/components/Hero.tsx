"use client"
import React from 'react'
import Image from 'next/image'
import Link from 'next/link';
import profile from "../../../public/Assets/Profile pic.jpg";
const Hero = () => {
    return (

        <section className="text-gray-600 body-font bg-orange-200">
            <div className="container mx-auto flex px-5 py-24 md:flex-row flex-col items-center">
                <div className="lg:flex-grow md:w-1/2 lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
                    <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-900">
                        I am
                        <br className="hidden lg:inline-block" />                       
                     Account Receptionist,and GIAIC Student
                    </h1>
                    <div className='w-[80px] h-[2px] bg-orange-700'></div>
                    <p className="mb-8 leading-relaxed">
                        My name is Warisha naz and I have done my BBA from PAF KIET University.Now I am Learning Next.Js and AI Metaverse.
                    </p>
                    <div className="flex justify-center">
                        
                        
                    <Link href={'#Contact'}>
                        <button className="ml-4 inline-flex text-white bg-orange-600 border-0 py-3 px-6 focus:outline-none hover:bg-orange-400 rounded text-lg">
                        
                            Contact
                        </button>
                        </Link>
                    </div>
                </div>
                <div className="lg:max-w-lg lg:w-full md:w-1/2 w-5/6">
                    <Image
                        className="object-cover object-center rounded mx-auto w-[18rem]"
                        width={150}
                        height={150}
                        alt="hero"
                        src={profile }
                       
                    />
                </div>
            </div>
        </section>


    )
}

export default Hero
